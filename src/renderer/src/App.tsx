...
function AutoStartToggle(): React.JSX.Element {
  ...
  const handleToggle = async () => {
    try {
      ...
      await window.api.setAutoStart(newValue)
      setAutoStart(newValue)
    } catch {
      console.error('Failed to toggle auto-start')
    }
  }
  ...
  if (error) {
    return (
      <div className="bg-zinc-950 rounded-lg p-4 border border-zinc-800 mb-4">
        <div className="text-zinc-500 text-xs">Auto-start not available in dev mode</div>
      </div>
    )
  }
  ...
  return (
    <div className="bg-zinc-950 rounded-lg p-4 border border-zinc-800 mb-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-medium text-white">Start on Login</h3>
          <p className="text-[10px] text-zinc-500 mt-0.5">Launch automatically when Windows starts</p>
        </div>
        <button
          onClick={handleToggle}
          disabled={loading}
          className={`relative w-11 h-6 rounded-full transition-colors ${autoStart ? 'bg-emerald-600' : 'bg-zinc-700'}`}
        >
          <span
            className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-white transition-transform ${autoStart ? 'translate-x-5' : 'translate-x-0'}`}
          />
        </button>
      </div>
    </div>
